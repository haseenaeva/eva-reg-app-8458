...
export const HorizontalOrganizationChart = ({
  ...
  const AgentCard = ({
    ...
    return <div className="flex flex-col items-center bg-gray-50">
        {/* Agent Box */}
        <Card className={`${getRoleColor(agent.role)} min-w-[220px] shadow-lg hover:shadow-2xl transition-all duration-300 border-2 border-white/30 backdrop-blur-sm`}>
          <CardContent className="p-4 text-center relative">
            {/* Expansion Toggle */}
            {hasSubordinates && <Button variant="ghost" size="sm" onClick={() => toggleNode(agent.id)} className="absolute top-2 left-2 h-6 w-6 p-0 text-white/80 hover:text-white hover:bg-white/20 rounded-full">
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex gap-1">
                <Button variant="ghost" size="sm" className="h-6 w-6 p-0 text-white/80 hover:text-white hover:bg-white/20 rounded-full">
                  <Edit className="h-3 w-3" />
                </Button>
                <Button variant="ghost" size="sm" className="h-6 w-6 p-0 text-white/80 hover:text-white hover:bg-red-500/30 rounded-full" onClick={() => {
            ...
            {hasSubordinates && <Badge variant="outline" className="bg-white/10 text-white border-white/30 text-xs mb-2">
                {subordinates.length} subordinate{subordinates.length !== 1 ? 's' : ''}
              </Badge>}
            
            {(agent.email || agent.phone) && <div className="mt-2 space-y-1">
                {agent.email && <div className="flex items-center justify-center gap-1 text-xs opacity-90">
                    <Mail className="h-3 w-3" />
                    <span className="truncate max-w-[140px]">{agent.email}</span>
                  </div>}
                {agent.phone && <div className="flex items-center justify-center gap-1 text-xs opacity-90">
        ...
        {hasSubordinates && isExpanded && <div className="flex flex-col items-center mt-6">
            {/* Vertical line down */}
            ...
            <div className="flex gap-12 mt-8">
              {subordinates.map(subordinate => <div key={subordinate.id} className="flex flex-col items-center">
  ...
  return <div className="space-y-8 bg-slate-50">
      {/* Controls Header */}
      <div className="flex items-center justify-between flex-wrap gap-4 p-4 bg-white rounded-lg shadow-md border">
        <div className="flex items-center gap-4">
          ...
          <div className="flex gap-2">
            <Button variant="outline" size="sm" onClick={expandAll} className="text-green-600 border-green-300 hover:bg-green-50">
              Expand All
            </Button>
            <Button variant="outline" size="sm" onClick={collapseAll} className="text-red-600 border-red-300 hover:bg-red-50">
              Collapse All
            </Button>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2 bg-gray-100 rounded-lg p-1">
            ...
            <Button variant="ghost" size="sm" onClick={handleZoomIn} disabled={zoomLevel >= 2} className="h-8 w-8 p-0">
              <ZoomIn className="h-4 w-4" />
            </Button>
            <Button variant="ghost" size="sm" onClick={resetZoom} className="text-xs px-2 h-8">
              Reset
            </Button>
          </div>
      ...
      {/* Panchayath Header */}
      <div className="bg-gradient-to-r from-slate-800 via-slate-900 to-slate-800 text-white p-8 rounded-xl shadow-2xl text-center border border-slate-700 bg-slate-50">
        <h1 className="text-4xl font-bold mb-3 tracking-wide text-cyan-400">{panchayathName.toUpperCase()}</h1>
      ...
      <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-8 rounded-xl overflow-auto shadow-2xl border border-slate-700">
        <div className="flex justify-center transition-transform duration-300 ease-in-out" style={{
        transform: `scale(${zoomLevel})`,
        transformOrigin: 'center top'
      }}>
          <div className="flex flex-col items-center gap-12">
            {coordinators.map(coordinator => <AgentCard key={coordinator.id} agent={coordinator} />)}
          </div>
        </div>
      </div>
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              {getSubordinates(agentToDelete?.id || '').length > 0 && <span className="block mt-2 text-orange-600 font-medium">
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={() => agentToDelete && deleteAgent(agentToDelete)}>
              Delete
            </Button>
          </DialogFooter>
        </DialogContent>
